...
export default function Settings() {
  ...
  const addAccount = async () => {
    ...
    try {
      const res = await fetch('https://api.mitradacademy.mg/api/accounts', {
        method: 'POST',
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`https://api.mitradacademy.mg/api/accounts/${editTarget.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('mitrad_token') },
  ...
  const confirmDelete = async () => {
    ...
    if (user?.password_set) {
      ...
      const loginRes = await fetch('https://api.mitradacademy.mg/api/login', {
        method: 'POST',
    ...
    try {
      const res = await fetch(`https://api.mitradacademy.mg/api/accounts/${deleteTarget}`, {
        method: 'DELETE',
  ...
  const handleSetPassword = async () => {
    ...
    try {
      const url = user?.password_set
        ? 'https://api.mitradacademy.mg/api/profile/password'
        : 'https://api.mitradacademy.mg/api/profile/set-password';
