...
export default function CalendarPage() {
  ...
  const weeks = useMemo(() => {
    ...
    // Construire une grille compl�te de 7 colonnes par semaine
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Vue d'ensemble de tes journ�es de trading</p>
        </div>
        <DisplayModeToggle />
      </div>
      ...
      <GlassCard className="animate-fade-up">
        ...
        <div className="flex items-center flex-wrap gap-3 mt-5 pt-4 border-t border-border/30">
          {[{ bg: 'bg-success/30 border-success/40', label: 'Positif' }, { bg: 'bg-destructive/30 border-destructive/40', label: 'N�gatif' }, { bg: 'bg-warning/30 border-warning/40', label: 'Breakeven' }].map(item => (
              ...
              {selectedDayTrades.map(t => (
                <div key={t.id} className="p-3 rounded-xl bg-accent/40 border border-border/30 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-right">
                    <div>
                      <p className={`metric-value text-sm ${t.resultR != null && t.resultR >= 0 ? 'text-success' : 'text-destructive'}`}>{t.resultR != null ? (t.resultR >= 0 ? '+' : '') + t.resultR.toFixed(2) + 'R' : 'RR � d�finir'}</p>
