...
export default function CalendarPage() {
  ...
  const monthWR     = closedMonth.length
    ? (closedMonth.filter(t => t.status === 'WIN').length / closedMonth.length * 100) : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <GlassCard className="animate-fade-up stagger-2 !p-5 relative overflow-hidden">
          <div className={`absolute inset-0 pointer-events-none rounded-2xl ${
            monthR >= 0 ? 'bg-gradient-to-br from-success/8 to-transparent' : 'bg-gradient-to-br from-destructive/8 to-transparent'
          }`} />
          <div className={`w-8 h-8 rounded-xl flex items-center justify-center mb-2 ${monthR >= 0 ? 'bg-success/20' : 'bg-destructive/20'}`}>
            <svg width="14" height="14" fill="none" stroke={monthR >= 0 ? '#00D4AA' : '#FF4757'} strokeWidth="2" viewBox="0 0 24 24">
          ...
          <p className={`metric-value text-2xl mt-0.5 ${monthR >= 0 ? 'text-success' : 'text-destructive'}`}>
            {formatResult(monthR, monthDollar, capital)}
          </p>
        </GlassCard>
        ...
        <GlassCard className="animate-fade-up stagger-4 !p-5 relative overflow-hidden">
          ...
          <div className="w-8 h-8 rounded-xl bg-purple-500/15 flex items-center justify-center mb-2">
            <svg width="14" height="14" fill="none" stroke="#6C3AFF" strokeWidth="2" viewBox="0 0 24 24">
      ...
      <GlassCard className="animate-fade-up">
        <div className="flex items-center justify-between mb-6">
          <button onClick={prevMonth} className="w-9 h-9 rounded-xl glass flex items-center justify-center hover:bg-accent/60 transition-all text-muted-foreground hover:text-foreground">
            <ChevronLeft size={16} />
          </button>
          <h2 className="text-lg font-bold text-foreground capitalize">
            {currentDate.toLocaleDateString('fr', { month: 'long', year: 'numeric' })}
          </h2>
          <button onClick={nextMonth} className="w-9 h-9 rounded-xl glass flex items-center justify-center hover:bg-accent/60 transition-all text-muted-foreground hover:text-foreground">
              ...
              <div key={day} className="relative">
                <button
                  ...
                  className={`min-h-[48px] sm:min-h-[100px] w-full rounded-xl border p-1 sm:p-1.5 text-xs transition-all duration-200 flex flex-col items-center justify-start relative
                    ...
                    ${isToday && !hasTrades ? 'border-primary/30 bg-primary/10' : ''}`}
                  style={{ border: borderStyle }}
                >
                  <span className={`font-semibold leading-tight self-start ${isToday ? 'text-primary' : hasTrades ? 'text-foreground' : 'text-muted-foreground/60'}`}>
                    {day}
                  </span>
                  {hasTrades && (
                    <>
                  ...
                  {!hasTrades && (
                    <div className="w-1.5 h-1.5 rounded-full bg-white/10 mx-auto mt-6" />
                  )}
                </button>

                {/* Tooltip au hover */}
                {isHovered && hasTrades && (
                  <div className="absolute z-50 bottom-[calc(100%+8px)] left-1/2 -translate-x-1/2 pointer-events-none"
                    style={{ opacity: 1, transition: 'opacity 150ms', minWidth: 180 }}>
                    <div className="rounded-xl p-3 shadow-[0_8px_32px_rgba(0,0,0,0.6)]"
                      style={{ background:'#0A0D16', border:'1px solid rgba(255,255,255,0.08)', borderRadius:12 }}>
                      <p className="text-xs font-semibold text-foreground mb-2">
                        {new Date(key + 'T12:00:00').toLocaleDateString('fr', { weekday:'short', day:'numeric', month:'short' })}
                      </p>
                      {dayTrades.map(t => (
                      ...
                      <div className="border-t border-white/8 mt-2 pt-1.5 flex justify-between">
                        ...
                        <span className={`text-xs font-bold ${dayR >= 0 ? 'text-success' : 'text-destructive'}`}>
        ...
        <div className="flex items-center flex-wrap gap-3 mt-5 pt-4 border-t border-border/30">
          {[
            ...
            { bg: 'bg-warning/30 border-warning/40', label: 'Breakeven' },
          ].map(item => (
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-bold text-foreground capitalize">
                  {new Date(selectedDay + 'T12:00:00').toLocaleDateString('fr', { weekday: 'long', month: 'long', day: 'numeric' })}
                </h3>
