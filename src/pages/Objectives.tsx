...
// ─────────────────────────────────────────────────────────────────────────────
// MODAL AJOUT / ÉDITION D'OBJECTIF
// ─────────────────────────────────────────────────────────────────────────────
function ObjectiveModal({
  ...
  const handleSubmit = () => {
    if (!text.trim()) { toast.error('Le titre est requis'); return; }
    onSave({ text: text.trim(), description: description.trim(), targetDate, image });
    onClose();
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Ex : Atteindre +20R ce mois-ci"
              autoFocus
            />
          </div>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground">
              Description <span className="font-normal opacity-40">(optionnel)</span>
            </label>
            <textarea
              value={description}
              onChange={e => setDescription(e.target.value)}
              className="input-dark mt-1.5 min-h-[70px] resize-none"
              placeholder="Détails, stratégie, sous-objectifs..."
            />
          </div>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
              <Calendar size={11} />
              Date cible <span className="font-normal opacity-40">(optionnel)</span>
            </label>
            <input
              type="date"
              value={targetDate}
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground">
              Image <span className="font-normal opacity-40">(optionnel, max 5 Mo)</span>
            </label>
            {image ? (
              <div className="relative mt-1.5 group/img overflow-hidden rounded-xl">
                <img src={image} alt="" className="w-full h-28 object-cover rounded-xl" />
                <button
                  onClick={() => setImage(undefined)}
                  className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/60 backdrop-blur-sm flex items-center justify-center hover:bg-destructive/80 transition-colors"
                >
              ...
              <label
                className="mt-1.5 flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all"
                style={{ border: '1.5px dashed rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.02)' }}
                onMouseEnter={e => {
              ...
              >
                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
                  ...
                  <ImagePlus size={15} style={{ color: '#a5b4fc' }} />
                </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          >
            <Save size={13} /> {initial ? 'Enregistrer' : 'Ajouter'}
          </button>
        </div>
      </div>
    </div>
