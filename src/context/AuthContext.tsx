...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [accounts, setAccounts] = useState<TradingAccount[]>(
    savedUser?.isDemo ? [{ id: 'demo-account', name: 'Compte D�mo', type: 'D�mo', capital: 10000, currency: 'USD' } as any] : []
  );
  ...
  // Recharge le profil frais depuis le serveur au d�marrage
  useEffect(() => {
  ...
  const updateProfile = (updates: any) => {
    ...
    localStorage.setItem('mitrad_user', JSON.stringify(updated)); // compatibilit�
    setUser(updated);
